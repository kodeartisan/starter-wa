...
export const exportToHtml = async ({
  ...
  // ++ MODIFIED: Added extensive styling for the header, messages, and quoted replies.
  const styles = `
    <style>
      ...
      .message-cluster { display: flex; flex-direction: column; margin-bottom: 2px; padding: 0 9%; scroll-margin-top: 20px; } /* Added scroll-margin-top */
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    // ++ ADDED: Generate the HTML for the quoted message reply.
    const quotedHtml = renderQuotedMessage(msg.quotedMsg)

    // ++ MODIFIED: Added id="message-${msg.id}" for linking and included the quotedHtml.
    htmlContent += `
