...
const text = async (message: string, number: string) => {
  ...
  // 2. Then, process variables like {name}, {number}, and dates on the varied message
  return _.chain(spintaxedMessage)
    ...
    .thru((str) =>
      _.includes(str, '{date}')
        ? _.replace(str, '{date}', format(new Date(), 'yyyy-MM-dd'))
        : str,
    )
    .thru((str) =>
      _.includes(str, '{time}')
        ? _.replace(str, '{time}', format(new Date(), 'HH:mm:ss'))
        : str,
    )
    .value()
}
