...
const useInputMessage = (options?: { isEditing?: boolean }) => {
  const form = useForm({
    ...
    validate: {
      ...
      inputImage: {
        file: (value, parent) => {
          if (options?.isEditing && parent.type === Message.IMAGE) {
            return null
          }
      ...
      inputVideo: {
        file: (value, parent) => {
          if (options?.isEditing && parent.type === Message.VIDEO) {
            return null
          }
      ...
      inputAudio: {
        file: (value, parent) => {
          if (options?.isEditing && parent.type === Message.AUDIO) {
            return null
          }
      ...
      inputFile: {
        file: (value, parent) => {
          if (options?.isEditing && parent.type === Message.FILE) {
            return null
          }
